defmodule Camp1Web.PostControllerTest do
  use Camp1Web.ConnCase

  alias Camp1.Topics

  @create_attrs %{current_content: "some current_content", original_content: "some original_content", parent_id: 42, status: "some status", type: "some type"}
  ...
  def fixture(:post) do
    {:ok, post} = Topics.create_post(@create_attrs)
    post
  end
  ...
  describe "update post" do
    ...
    test "redirects when data is valid", %{conn: conn, post: post} do
      ...
      assert html_response(conn, 200) =~ "some updated current_content"
    end
